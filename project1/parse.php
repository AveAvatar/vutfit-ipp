...
$array_case6 = array('ADD', 'SUB', 'MUL', 'IDIV', 'LT', 'GT', 'EQ', 'OR', strtoupper('aND'), 'STRI2INT', 'CONCAT', 'GETCHAR', 'SETCHAR');
...
while ($line = fgets(STDIN)) {
    ...
    if (!$started) {
        $line = trim($line, " ");
        if($line == ".IPPcode22" or $line == ".IPPcode22\n") {
    ...
    if($parse == true) {
        ...
        switch ($splitted[0]) { 
            ...
            default:          
                if($splitted[0] == strtoupper(".IPPCODE22") && $started == false) {
                    $started = true; 
                    break;
